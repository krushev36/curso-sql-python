---
title: Notebook 08: Proyecto Integrador
---

Fuente original: [08_proyecto_integrador.sql](https://github.com/krushev36/curso-sql-python/blob/main/sql/notebooks/08_proyecto_integrador.sql)

# Notebook 08: Proyecto Integrador
## Fundamentos de Programación
### Maestría en Ciencia de Datos e Inteligencia de Negocios · Universidad de Antioquia
## 1. Bienvenida al Proyecto Final
Bienvenida y bienvenido al **capstone** del curso **Fundamentos de Programación** de la **Maestría en Ciencia de Datos e Inteligencia de Negocios** de la **Universidad de Antioquia**.
En este notebook asumirás el rol de **Data Analyst Senior en DataCorp Analytics** y entregarás un reporte integral para el comité ejecutivo (**CEO, CFO, CMO, COO y Chief Data Officer**). El objetivo es transformar datos transaccionales en decisiones de negocio accionables usando **consultas básicas, funciones SQL, agregaciones, JOINs, subconsultas, CTEs y funciones de ventana**.
### Entregable esperado
| Entregable | Descripción ejecutiva |
|---|---|
| Análisis 360 de clientes | LTV, retención, inactividad y segmentación |
| Análisis comercial | ventas, demanda, margen y proveedores |
| Análisis geográfico | regiones, países y concentración del ingreso |
| Análisis temporal | evolución mensual, tendencia y estabilidad |
| Dataset para ML | variables listas para modelos de churn o propensión |
| Dashboard SQL | reporte consolidado para la C-suite |
> **📝 Nota:** Este notebook está diseñado para demostrar integración de competencias. No se trata de una sola consulta “correcta”, sino de construir una narrativa analítica completa y defendible.

## 2. Objetivos de aprendizaje
Al finalizar este proyecto serás capaz de:
1. Integrar **todas las habilidades del curso** en un caso de negocio de nivel profesional.
2. Traducir preguntas ejecutivas en consultas SQL reproducibles y explicables.
3. Diseñar indicadores de clientes, productos, ventas, geografía y tiempo.
4. Aplicar **CTEs, subconsultas y funciones de ventana** para crear métricas avanzadas.
5. Preparar un **dataset analítico** reutilizable para modelos de Machine Learning.
6. Construir un **dashboard SQL** orientado a toma de decisiones.
### Mapa de aprendizaje del proyecto
```text
Pregunta de negocio
        -> Definición del grano
        -> Unión de tablas correctas
...
- Escritura de consultas SQL legibles y bien documentadas.
- Integración de tablas relacionales con claves de negocio.
- Construcción de métricas con `SUM`, `AVG`, `COUNT`, `CASE`, `COALESCE` y `NULLIF`.
- Uso de **subconsultas y CTEs** para organizar lógica analítica compleja.
- Aplicación de **window functions** como `ROW_NUMBER`, `DENSE_RANK`, `NTILE`, `LAG` y acumulados.
### Competencias analíticas
- Priorizar clientes por valor y riesgo.
- Detectar concentración comercial y geográfica.
- Relacionar demanda, margen y abastecimiento.
- Identificar tendencias temporales y señales tempranas de cambio.
### Competencias de comunicación ejecutiva
| Competencia | Evidencia en este notebook |
|---|---|
| Sintetizar hallazgos | tablas resumen y KPIs |
| Explicar supuestos | comentarios y notas metodológicas |
| Recomendar acciones | interpretación al cierre de cada módulo |
| Pensar en escalabilidad | vista final para ML y dashboard reutilizable |

## 4. Contexto empresarial
**DataCorp Analytics** es una firma global que comercializa productos a clientes empresariales en múltiples regiones. Durante el último comité trimestral, la dirección detectó cuatro necesidades críticas:
1. **Crecimiento con rentabilidad:** no basta vender más; hay que identificar qué clientes, productos y regiones generan valor sostenible.
2. **Retención y reactivación:** existe preocupación por clientes que dejaron de comprar y por segmentos cuyo comportamiento se está debilitando.
3. **Planeación comercial y operativa:** el área de compras necesita claridad sobre demanda y confiabilidad de proveedores.
4. **Preparación para analítica avanzada:** el área de datos quiere transformar los hallazgos SQL en variables listas para modelos de churn, scoring o recomendación.
### Preguntas del comité ejecutivo
- ¿Cuáles son los **10 clientes con mayor valor de vida**?
- ¿Qué **segmentos de clientes** generan más ingresos?
- ¿Cómo evoluciona la **venta mensual**?
- ¿Qué **productos** concentran la mayor demanda?
- ¿Qué **proveedores** son más confiables por volumen?
- ¿Dónde se concentran las ventas por **región**?
- ¿Qué clientes están **inactivos**?
- ¿Cuál es la **tasa de retención**?
- ¿Qué productos presentan **mayor margen estimado**?
- ¿Cómo preparar un **dataset para Machine Learning**?
> **📝 Nota:** En un contexto real, la diferencia entre un buen analista y un analista estratégico suele estar en su capacidad para conectar métricas aisladas con decisiones concretas de negocio.
...
nyctaxi.trips
   |
   -> dataset complementario para pensar patrones temporales y escalabilidad analítica
```
### Rol de cada dataset
| Tabla | Rol analítico | Grano principal |
|---|---|---|
| `samples.tpch.customer` | perfil del cliente | 1 fila por cliente |
| `samples.tpch.orders` | encabezado de pedido | 1 fila por pedido |
| `samples.tpch.lineitem` | detalle transaccional | 1 fila por línea de pedido |
| `samples.tpch.part` | catálogo de productos | 1 fila por producto |
| `samples.tpch.supplier` | catálogo de proveedores | 1 fila por proveedor |
| `samples.tpch.nation` | dimensión país | 1 fila por país |
| `samples.tpch.region` | dimensión región | 1 fila por región |
| `samples.nyctaxi.trips` | referencia temporal adicional | 1 fila por viaje |
### Regla metodológica clave
- Para métricas comerciales agregadas, el **grano correcto** depende de la pregunta.
- Si analizas ingreso por pedido, la base natural es `orders`.
- Si analizas producto, proveedor o volumen físico, la base natural es `lineitem`.
- Si analizas geografía de cliente, el camino es `customer -> nation -> region`.
...
## 6. Módulo 1: Análisis de Clientes
En este módulo responderemos preguntas sobre **valor, segmentación, inactividad y concentración de ingresos**.
### Hipótesis ejecutiva
Si identificamos qué clientes compran más, qué segmentos aportan más valor y quiénes muestran señales de abandono, podremos asignar presupuesto comercial con mayor precisión.
### Flujo analítico
```text
Cliente -> Pedido -> Ingreso -> Recencia/Frecuencia -> Prioridad comercial
```
...
### Lectura ejecutiva del Módulo 1
- El ranking de segmentos permite enfocar campañas y asignación de ejecutivos.
- El top 10 de LTV identifica cuentas estratégicas que requieren gestión diferenciada.
- La lista de clientes inactivos sirve como base para campañas de reactivación.
- La concentración por deciles ayuda a responder si el negocio depende de una base muy reducida de clientes.
> **📝 Nota:** Si observas alta concentración en pocos clientes, la prioridad estratégica no es solo vender más, sino **diversificar riesgo comercial**.

## 7. Módulo 2: Análisis de Ventas y Productos
Ahora pasamos del nivel cliente al nivel **producto, demanda y abastecimiento**.
### Supuesto metodológico
La tabla `lineitem` nos permite observar la realidad operativa: cantidades, descuentos, proveedores y líneas vendidas.
> **📝 Nota:** La tabla `part` no contiene costo real. Para aproximar margen construiremos un **margen estimado** usando un costo teórico equivalente al **65% del precio retail**. El objetivo es pedagógico: documentar un supuesto y volverlo transparente.
...
### Lectura ejecutiva del Módulo 2
- Los productos con más demanda ayudan a priorizar inventario, merchandising y negociación comercial.
- El margen estimado introduce una conversación clave: **no todo lo que más vende es lo que más rentabiliza**.
- El ranking de proveedores sirve para identificar aliados críticos y riesgos de dependencia.
- La relación producto-proveedor apoya decisiones de abastecimiento y continuidad operativa.

## 8. Módulo 3: Análisis Geográfico
En este módulo evaluaremos la distribución espacial del negocio: **regiones, países, densidad de clientes y concentración del ingreso**.
### Pregunta estratégica
¿La compañía está equilibrada globalmente o depende de unas pocas geografías?
...
### Lectura ejecutiva del Módulo 3
- Las regiones con mayor ingreso explican la actual concentración comercial.
- Los países líderes dentro de cada región sirven para focalizar expansión o defensa competitiva.
- Una región con muchos clientes pero bajo ingreso por cliente puede requerir mejora en pricing, cross-sell o cobertura comercial.
> **📝 Nota:** El análisis geográfico no solo dice “dónde vendemos”, sino también “dónde estamos subcapitalizando nuestra base instalada”.

## 9. Módulo 4: Análisis de Tendencias Temporales
En este módulo pasamos del estado actual a la **dinámica temporal**: crecimiento, desaceleración, retención y estacionalidad.
### Diagrama temporal
```text
Fecha de pedido -> Mes -> Ingreso -> Variación -> Tendencia -> Acción
```
...
### Lectura ejecutiva del Módulo 4
- La evolución mensual ayuda a distinguir crecimiento estructural de ruido de corto plazo.
- El promedio móvil reduce volatilidad y mejora la lectura gerencial.
- La retención mensual conecta ingresos con salud de la base de clientes.
- El ejemplo de `nyctaxi.trips` muestra cómo la misma lógica temporal puede escalar a otros dominios analíticos.

## 10. Módulo 5: Preparación de datos para Machine Learning
El objetivo ahora es crear un **dataset analítico por cliente** que pueda alimentar modelos de:
- churn o riesgo de fuga,
- scoring de valor,
- recomendación comercial,
- priorización de cuentas.
### Principio de diseño
Una buena tabla para ML debe tener una fila por entidad objetivo y columnas numéricas o categóricas que resuman comportamiento histórico.
...
## 11. Dashboard SQL (reporte integral de negocio)
En esta sección condensamos los hallazgos en un formato cercano a un **dashboard ejecutivo**.
### Estructura del dashboard
```text
KPIs globales
   + matriz región-segmento
   + watchlist comercial
   + panel de productos y proveedores
```
> **📝 Nota:** En Databricks, estas salidas pueden convertirse fácilmente en visualizaciones nativas: tarjetas KPI, barras, líneas, mapas o tablas con formato condicional.
...
## 12. Desafíos adicionales
Si quieres llevar el proyecto más lejos, intenta resolver estos desafíos por tu cuenta:
1. Construir una **cohorte de clientes** por mes de primera compra y medir supervivencia a 3, 6 y 12 meses.
2. Crear un **índice RFM** completo (`Recency`, `Frequency`, `Monetary`) y clasificar clientes premium.
3. Detectar **anomalías de pedidos** usando desviación estándar o percentiles por segmento y mes.
4. Diseñar un **ranking de riesgo de abastecimiento** considerando concentración de proveedor por producto.
5. Crear una tabla de **contribución acumulada** de productos para encontrar el equivalente al principio 80/20.
6. Integrar una serie temporal de `samples.nyctaxi.trips` con ventanas móviles de 7 y 30 periodos.
A continuación se muestran dos desafíos resueltos como ejemplo de extensión analítica.
...
A lo largo de los ocho notebooks recorriste un camino completo:
| Notebook | Competencia principal | Evidencia en este proyecto |
|---|---|---|
| 01 | exploración inicial | lectura del ecosistema Databricks |
| 02 | consultas básicas | selección y filtrado de información |
| 03 | funciones SQL | limpieza, reglas y transformaciones |
| 04 | agregaciones | KPIs y métricas de negocio |
| 05 | JOINs | integración de tablas relacionales |
| 06 | subconsultas y CTEs | modularidad y análisis avanzado |
| 07 | window functions | rankings, tendencias y concentración |
| 08 | proyecto integrador | narrativa ejecutiva completa |
### Idea central del curso
**SQL no es solo un lenguaje para consultar datos; es una herramienta para pensar, estructurar y comunicar decisiones basadas en evidencia.**

## 14. Reflexión final y próximos pasos
Ya dominas el corazón del análisis relacional en Databricks SQL. El siguiente paso natural es integrar este trabajo con **Python y PySpark** para automatizar pipelines, entrenar modelos y publicar activos analíticos reutilizables.
### Vista previa de integración
```python
# Ejemplo conceptual en un siguiente módulo práctico
df_ml = spark.sql("SELECT * FROM dataset_ml_clientes")
display(df_ml)
# Luego podrías:
# 1. limpiar valores faltantes,
# 2. entrenar un modelo de churn,
# 3. escribir predicciones en una tabla Delta,
# 4. volver a consumirlas desde SQL.
```
### Próximos pasos sugeridos
1. Convertir vistas temporales en tablas curadas.
2. Orquestar actualizaciones periódicas con jobs de Databricks.
3. Conectar dashboards SQL con modelos de scoring en PySpark.
4. Publicar resultados a usuarios de negocio mediante visualizaciones y alertas.
> **📝 Nota:** El perfil más valioso en ciencia de datos aplicada suele combinar tres capacidades: **SQL para entender el negocio, Python para automatizar y Spark para escalar**.

## 15. Autoevaluación final
Responde estas preguntas antes de cerrar el proyecto:
1. ¿Puedo explicar con claridad la diferencia entre el grano de `orders` y el de `lineitem`?
2. ¿Sé justificar por qué una métrica debe agregarse por cliente, producto, región o mes?
3. ¿Entiendo cuándo conviene usar una CTE, una subconsulta o una función de ventana?
4. ¿Puedo documentar supuestos analíticos, como el uso de un margen estimado?
5. ¿Sería capaz de convertir este notebook en una vista o pipeline productivo?
### Checklist de dominio
- [ ] Puedo construir consultas ejecutivas sin ambigüedad en el grano.
- [ ] Puedo identificar clientes inactivos, segmentos líderes y productos críticos.
- [ ] Puedo medir retención, concentración y tendencia temporal.
- [ ] Puedo preparar un dataset analítico listo para ML.
- [ ] Puedo comunicar hallazgos de forma profesional a una audiencia no técnica.

## ¡Felicitaciones!
Has completado el **Proyecto Integrador** del curso **Fundamentos de Programación** de la **Maestría en Ciencia de Datos e Inteligencia de Negocios** de la **Universidad de Antioquia**.
Cerraste el ciclo completo: desde consultas básicas hasta un reporte de inteligencia de negocio listo para una **C-suite**. Este es exactamente el tipo de trabajo que convierte a SQL en una competencia estratégica dentro de un programa de **Maestría en Ciencia de Datos e Inteligencia de Negocios**.
**Siguiente meta:** llevar estos resultados a notebooks híbridos con **SQL + Python + PySpark**, construir features en producción y conectar análisis descriptivo con modelos predictivos.
¡Excelente trabajo!
