---
title: Notebook 02: SELECT y Consultas Básicas
---

Fuente original: [02_select_consultas_basicas.sql](https://github.com/krushev36/curso-sql-python/blob/main/sql/notebooks/02_select_consultas_basicas.sql)

# Notebook 02: SELECT y Consultas Básicas
## Fundamentos de Programación
### Maestría en Ciencia de Datos e Inteligencia de Negocios · Universidad de Antioquia
**Semana 2**
**Rol del estudiante:** Data Analyst en **DataCorp Analytics**
**Misión del día:** responder solicitudes básicas del gerente usando consultas `SELECT` sobre clientes, órdenes y catálogos operativos.
> **📝 Nota:** Este notebook está diseñado para ejecutarse en Databricks SQL. Todas las consultas usan únicamente tablas públicas del catálogo `samples`.

## 1. Bienvenida
En esta segunda semana pasamos de la exploración del entorno a la **consulta directa de datos**. La sentencia `SELECT` es la puerta de entrada a casi todo análisis en SQL: permite elegir columnas, filtrar filas, ordenar resultados y limitar salidas.
En el contexto de **DataCorp Analytics**, tu gerente necesita un reporte preliminar con información de clientes, órdenes y catálogos. Antes de construir métricas complejas, debes dominar las consultas básicas con precisión.
```text
Necesidad del negocio
       │
       ├── ¿Qué columnas necesito?
       ├── ¿Qué filas cumplen la condición?
       ├── ¿Cómo presento el resultado?
...
1. Identificar la anatomía completa de una consulta `SELECT`.
2. Diferenciar entre `SELECT *` y la selección explícita de columnas.
3. Crear alias legibles con `AS` para mejorar reportes.
4. Aplicar `DISTINCT` para obtener valores únicos.
5. Filtrar filas con `WHERE`, operadores de comparación y operadores lógicos.
6. Usar `BETWEEN`, `IN`, `LIKE`, `IS NULL` e `IS NOT NULL`.
7. Ordenar y recortar resultados con `ORDER BY` y `LIMIT`.
8. Interpretar tipos de datos básicos en tablas analíticas.

## 3. Competencias
| Competencia | Descripción aplicada al curso | Evidencia esperada |
|---|---|---|
| Lectura de datos | Reconoce tablas, columnas y tipos de datos relevantes | Consulta columnas correctas sin ambigüedad |
| Pensamiento analítico | Traduce preguntas del negocio a filtros SQL | Usa `WHERE` con condiciones claras |
| Comunicación técnica | Presenta resultados con nombres entendibles | Usa alias con `AS` |
| Calidad analítica | Reduce ruido y evita duplicados innecesarios | Aplica `DISTINCT`, `ORDER BY` y `LIMIT` |
| Validación | Detecta errores de sintaxis y de lógica | Explica por qué una consulta funciona |

## 4. Contexto empresarial
El gerente comercial de **DataCorp Analytics** pidió un corte rápido de información para una reunión regional:
- identificar clientes relevantes,
- revisar órdenes por estado,
- explorar segmentos de mercado,
- observar catálogos de productos y proveedores,
- preparar una muestra pequeña para el reporte ejecutivo.
Tu trabajo no es todavía unir todas las tablas, sino **consultar con precisión cada fuente** y devolver resultados limpios, entendibles y verificables.
> **📝 Nota:** En ciencia de datos, muchas fallas del análisis comienzan por una mala consulta base: columnas innecesarias, filtros ambiguos o resultados sin ordenar.

## 5. Conceptos
### 5.1 Anatomía de una consulta `SELECT`
```text
...
LIMIT n
```
### 5.2 ¿Qué hace cada cláusula?
| Cláusula | Función | Pregunta que responde |
|---|---|---|
| `SELECT` | Elige columnas o expresiones | ¿Qué quiero ver? |
| `FROM` | Indica la tabla de origen | ¿De dónde sale la información? |
| `WHERE` | Filtra filas | ¿Qué registros cumplen la condición? |
| `ORDER BY` | Ordena el resultado | ¿Cómo presento primero lo más importante? |
| `LIMIT` | Restringe el número de filas | ¿Cuántas filas necesito inspeccionar? |
### 5.3 Tipos de datos básicos en SQL
| Tipo | Ejemplo | Uso frecuente |
|---|---|---|
| Entero (`INT`, `BIGINT`) | `c_custkey`, `o_orderkey` | Identificadores y conteos |
| Decimal (`DECIMAL`, `DOUBLE`) | `c_acctbal`, `o_totalprice` | Importes, saldos, distancias |
| Texto (`STRING`) | `c_name`, `o_orderstatus` | Nombres, estados, comentarios |
| Fecha (`DATE`) | `o_orderdate` | Seguimiento temporal |
> **📝 Nota:** El tipo de dato define qué operaciones tienen sentido. Comparar fechas como texto o sumar columnas de texto suele producir errores o resultados engañosos.

## 5. Conceptos (continuación)
### 5.4 Operadores que usaremos hoy
| Categoría | Operadores | Uso |
|---|---|---|
| Comparación | `=`, `!=`, `<`, `>`, `<=`, `>=` | Comparar valores |
| Lógicos | `AND`, `OR`, `NOT` | Combinar condiciones |
| Rango | `BETWEEN ... AND` | Buscar dentro de un intervalo |
| Conjunto | `IN (...)` | Buscar dentro de una lista |
| Patrones | `LIKE`, `%`, `_` | Buscar coincidencias parciales |
| Nulos | `IS NULL`, `IS NOT NULL` | Detectar ausencia de dato |
### 5.5 Buenas prácticas iniciales
1. Evita `SELECT *` en reportes finales.
2. Usa alias para nombres comprensibles.
3. Ordena resultados cuando comparas extremos.
4. Limita filas durante la exploración.
5. Lee el error: casi siempre indica la cláusula problemática.
> **📝 Nota:** Un error muy común es escribir `WHERE columna = NULL`. En SQL, la comparación correcta es `WHERE columna IS NULL`.

## 6. Explicación paso a paso
En esta sección iremos de lo más básico a lo más útil para un analista junior:
...
### Paso 1: `SELECT` específico vs `SELECT *`
**¿Por qué no usar `SELECT *` aquí?**
- porque trae columnas que quizá no necesitas,
- consume más lectura y hace más difícil validar resultados,
- vuelve frágil un reporte si la estructura de la tabla cambia.
**Resultado esperado:** una vista pequeña, clara y útil para validar la tabla `customer`.
...
### Paso 2: Alias con `AS`
Los alias ayudan cuando:
- el nombre original es críptico,
- quieres estandarizar vocabulario del negocio,
- necesitas exportar resultados a usuarios no técnicos.
> **📝 Nota:** Aunque `AS` es opcional en muchos motores, usarlo mejora legibilidad y reduce confusiones.
...
### Paso 3: `DISTINCT`
`DISTINCT` responde preguntas como:
- ¿qué estados existen?
- ¿qué segmentos de mercado aparecen?
- ¿qué modos de pago están registrados?
**Resultado esperado:** un pequeño catálogo de estados, normalmente con muy pocas filas.
...
### Paso 4: `WHERE` y operadores de comparación
Observa cómo cambia el significado con cada operador:
| Operador | Significado | Ejemplo |
|---|---|---|
| `=` | Igual a | `c_mktsegment = 'AUTOMOBILE'` |
| `!=` | Distinto de | `o_orderstatus != 'F'` |
| `>` | Mayor que | `c_acctbal > 5000` |
| `<=` | Menor o igual que | `o_totalprice <= 100000` |
**Resultado esperado:** pocas filas, enfocadas en un criterio de negocio concreto.
...
### Paso 5: `AND`, `OR`, `NOT`, `BETWEEN` e `IN`
Estas expresiones son útiles cuando una pregunta del negocio tiene varias condiciones al mismo tiempo.
**Lectura lógica de la consulta anterior:**
1. la orden debe ser de 1995,
2. el estado debe pertenecer a una lista válida,
3. el valor no puede ser bajo,
4. la prioridad debe ser urgente o alta.
> **📝 Nota:** Siempre que mezcles `AND` y `OR`, usa paréntesis para dejar explícita la intención lógica.
...
### Paso 6: `LIKE`, `IS NULL`, `IS NOT NULL`, `ORDER BY` y `LIMIT`
| Elemento | Idea clave | Ejemplo mental |
|---|---|---|
| `LIKE 'ABC%'` | Empieza por `ABC` | prefijos |
| `LIKE '%ABC%'` | Contiene `ABC` | búsqueda parcial |
| `LIKE 'A_C'` | Un solo carácter intermedio | patrón fijo |
| `IS NULL` | Falta el dato | ausencia real |
| `IS NOT NULL` | El dato sí existe | completitud |
| `ORDER BY ... DESC` | De mayor a menor | top valores |
| `LIMIT 10` | Solo una muestra | revisión rápida |
**Error frecuente:** ordenar una columna distinta de la que el negocio quiere priorizar.

## 7. Ejemplo completamente explicado
A continuación verás **cinco ejemplos completos**. Cada uno incluye el motivo de negocio, la consulta comentada y la interpretación esperada.
...
## 8. Ejemplo guiado
En los siguientes cinco casos ya conoces la intención de negocio y la estructura general. La idea es que leas la explicación, ejecutes la consulta y luego modifiques el filtro por tu cuenta.
...
## 9. Ejercicio guiado
**Instrucción:** intenta resolver cada reto antes de mirar el resultado. Luego compara tu versión con la solución propuesta.
| Nivel | Reto |
|---|---|
| Muy Fácil | Mostrar 10 naciones con alias legibles |
| Fácil | Listar regiones únicas ordenadas |
| Intermedio | Filtrar clientes de dos segmentos |
| Intermedio Alto | Buscar órdenes no cerradas y de precio alto |
| Desafío | Detectar filas con valor nulo en una columna derivada |
...
Resuelve estos retos **sin ver ayuda adicional**. El nivel progresa de **Muy Fácil** a **Desafío**.
| Nivel | Ejercicio | Pista |
|---|---|---|
| Muy Fácil | Mostrar 8 clientes con `SELECT` explícito y `LIMIT` | Evita `SELECT *` |
| Fácil | Obtener estados únicos de órdenes con `DISTINCT` | Usa `ORDER BY` |
| Intermedio | Filtrar partes con precio entre 900 y 1100 | Usa `BETWEEN` |
| Intermedio Alto | Buscar proveedores con saldo distinto de 0 y nombre tipo `Supplier#00001%` | Combina `!=` y `LIKE` |
| Desafío | Mostrar órdenes de 1996, prioridad alta y estado en una lista específica | Mezcla `BETWEEN`, `IN` y `AND` |
> **📝 Nota:** Antes de ejecutar, intenta escribir la consulta en papel identificando `SELECT`, `FROM`, `WHERE`, `ORDER BY` y `LIMIT`.

## 11. Desafío
Ahora piensa como analista de negocio. Cada reto exige que decidas **qué columna ver**, **cómo filtrar** y **cómo presentar** el resultado.
| Nivel | Desafío analítico |
|---|---|
| Muy Fácil | Listar regiones en orden descendente |
| Fácil | Encontrar clientes cuyo nombre contenga `009` |
| Intermedio | Mostrar órdenes con total menor a 75000 y estado distinto de `F` |
| Intermedio Alto | Encontrar proveedores con saldo entre 5000 y 8000 y comentario no nulo |
| Desafío | Diseñar una consulta que muestre piezas de ciertos tamaños y precios, ordenadas por fabricante y precio |
**Criterios de éxito:**
1. columnas pertinentes,
2. filtros correctos,
3. alias claros,
4. orden lógico,
5. salida legible.
...
## 12. Resumen
Hoy consolidaste la base operativa de cualquier analista SQL:
- `SELECT` para elegir columnas,
- `FROM` para indicar la tabla,
- `WHERE` para filtrar,
- `AS` para comunicar mejor,
- `DISTINCT` para eliminar repetición,
- operadores de comparación y lógicos para traducir reglas de negocio,
- `ORDER BY` y `LIMIT` para presentar resultados útiles.
### Regla de oro
```text
Primero claridad,
luego precisión,
luego velocidad.
```
Si una consulta no es clara, será difícil confiar en el análisis posterior.

## 13. Laboratorio
**Escenario:** tu gerente necesita respuestas rápidas para un reporte ejecutivo de media mañana. Debes responder con consultas simples, legibles y listas para exportar.
**Preguntas reales de negocio:**
1. ¿Qué clientes del segmento `MACHINERY` tienen mayor saldo?
2. ¿Qué órdenes abiertas o pendientes superan 300000?
3. ¿Qué líneas de pedido tienen descuentos entre 0.05 y 0.07?
4. ¿Qué partes del fabricante `Manufacturer#1` tienen precio de lista bajo?
5. ¿Qué proveedores tienen saldo alto y teléfono registrado?
...
## 14. Autoevaluación
Responde sin ejecutar código. Si dudas, vuelve a la sección correspondiente.
1. ¿Cuándo conviene evitar `SELECT *`?
2. ¿Qué ventaja aporta `AS` en un reporte ejecutivo?
3. ¿Qué diferencia hay entre `DISTINCT columna` y `DISTINCT columna1, columna2`?
4. ¿Qué operador usarías para excluir un estado: `=` o `!=`?
5. ¿Cuándo usarías `AND` y cuándo `OR`?
6. ¿Qué ventaja tiene `IN (...)` frente a múltiples comparaciones con `OR`?
7. ¿Qué diferencia existe entre `LIKE 'AB%'` y `LIKE 'AB_'`?
8. ¿Por qué `columna = NULL` no es correcto en SQL?
9. ¿Qué efecto tiene `ORDER BY ... DESC` en la interpretación del resultado?
10. ¿Para qué sirve `LIMIT` durante la exploración inicial?
> **📝 Nota:** Si puedes explicar en voz alta la lógica de una consulta simple, entonces ya estás avanzando de “escribir SQL” a “pensar analíticamente con SQL”.
