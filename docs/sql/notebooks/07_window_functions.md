---
title: Notebook 07: Window Functions
---

Fuente original: [07_window_functions.sql](https://github.com/krushev36/curso-sql-python/blob/main/sql/notebooks/07_window_functions.sql)

# Notebook 07: Window Functions
## Fundamentos de Programación
### Maestría en Ciencia de Datos e Inteligencia de Negocios · Universidad de Antioquia
## 1. Bienvenida
Bienvenidos al séptimo notebook del curso **Fundamentos de Programación** de la **Maestría en Ciencia de Datos e Inteligencia de Negocios** de la **Universidad de Antioquia**.
En esta sesión asumirás el rol de **Data Analyst en DataCorp Analytics**. El CFO necesita análisis avanzados que no destruyan el detalle original: acumulados, rankings, comparaciones mes contra mes, percentiles y señales tempranas de tendencia.
Las **window functions** resuelven exactamente ese problema: permiten calcular métricas analíticas **sin colapsar las filas** como ocurre con `GROUP BY`.
### Idea central
| Pregunta del negocio | ¿Basta con `GROUP BY`? | ¿Se beneficia de ventanas? |
|---|---|---|
| ¿Cuál es el total por región? | Sí | A veces |
| ¿Cómo se compara cada pedido con el promedio de su región? | No | Sí |
| ¿Cuál es el ranking de clientes dentro de cada segmento? | No | Sí |
| ¿Cuál es el acumulado mes a mes sin perder el mes individual? | No | Sí |
> **📝 Nota:** Piensa en una ventana como un **subconjunto dinámico de filas relacionadas** con la fila actual. SQL calcula sobre ese subconjunto y luego devuelve el resultado en la misma fila.

## 2. Objetivos de aprendizaje
Al finalizar este notebook podrás:
1. Explicar qué es una función de ventana y por qué es más poderosa que una agregación simple en ciertos escenarios.
2. Describir la anatomía de `OVER()` y el papel de `PARTITION BY`, `ORDER BY` y el frame.
3. Aplicar funciones de ranking: `ROW_NUMBER()`, `RANK()`, `DENSE_RANK()` y `NTILE()`.
4. Aplicar funciones de desplazamiento: `LAG()`, `LEAD()`, `FIRST_VALUE()`, `LAST_VALUE()` y `NTH_VALUE()`.
5. Construir acumulados, medias móviles, porcentajes del total y percentiles.
6. Usar `QUALIFY` en Databricks para filtrar directamente resultados basados en funciones de ventana.
7. Diseñar variables derivadas útiles para *feature engineering* en proyectos de analítica avanzada y ML.

## 3. Competencias
### Competencias técnicas
- Construir consultas con ventanas manteniendo el nivel de detalle correcto.
- Seleccionar la partición y el orden adecuados según la pregunta de negocio.
- Elegir el frame correcto para evitar resultados engañosos.
### Competencias analíticas
- Detectar líderes, rezagos, cambios de tendencia y concentración de ingresos.
- Traducir preguntas ejecutivas a patrones analíticos reproducibles.
- Comparar individuos contra su grupo de referencia sin perder granularidad.
### Competencias profesionales
| Competencia | Evidencia esperada |
|---|---|
| Comunicación analítica | Explicas por qué la ventana responde mejor la pregunta |
| Calidad SQL | Comentarios claros, alias semánticos y orden lógico |
| Pensamiento crítico | Detectas errores de partición, orden y frame |

## 4. Contexto empresarial
En **DataCorp Analytics**, el CFO revisa desempeño comercial, estabilidad de ingresos y riesgos de concentración. El problema es que varios reportes actuales solo entregan agregados finales y no permiten responder preguntas como:
1. ¿Qué pedido está muy por encima del promedio de su segmento?
2. ¿Qué clientes vienen perdiendo dinamismo mes a mes?
3. ¿Qué regiones explican la mayor parte del ingreso total?
4. ¿Qué proveedores o productos están en percentiles extremos?
5. ¿Qué variables históricas pueden alimentar un modelo predictivo?
### Datasets del notebook
| Dataset | Uso analítico principal |
|---|---|
| `samples.tpch.customer` | segmentos, cuentas y clientes |
| `samples.tpch.orders` | historial de pedidos y valor monetario |
| `samples.tpch.lineitem` | detalle transaccional por ítem |
| `samples.tpch.part` | catálogo de productos |
| `samples.tpch.supplier` | proveedores |
| `samples.tpch.nation` | país |
| `samples.tpch.region` | región |
| `samples.nyctaxi.trips` | referencia opcional para series temporales urbanas |
> **📝 Nota:** En este notebook trabajaremos principalmente con TPCH porque ofrece relaciones claras entre clientes, pedidos, productos, proveedores y regiones.

## 5. Conceptos
### 5.1 ¿Qué es una window function?
Una función de ventana calcula un valor usando varias filas relacionadas con la fila actual, **pero devuelve el resultado sin agrupar ni eliminar filas**.
```text
GROUP BY colapsa filas
cliente  pedido  valor    ->   cliente  total
A        1       100          A        250
A        2       150
Window function conserva filas
cliente  pedido  valor  total_cliente
A        1       100    250
A        2       150    250
```
### 5.2 Anatomía de `OVER()`
| Componente | Función | Ejemplo conceptual |
|---|---|---|
| `OVER()` | activa el comportamiento de ventana | `SUM(x) OVER()` |
| `PARTITION BY` | divide en grupos lógicos sin colapsar filas | `PARTITION BY region` |
| `ORDER BY` | define secuencia dentro de la ventana | `ORDER BY fecha` |
| `ROWS BETWEEN` / `RANGE BETWEEN` | define qué filas exactas entran al cálculo | acumulado, media móvil |
### 5.3 Tipos frecuentes
- **Ranking:** posición relativa dentro de un grupo.
- **Offset:** comparación con filas previas o siguientes.
- **Agregadas sobre ventana:** suma, promedio, conteo, mínimo, máximo por grupo o recorrido.
- **Percentiles y distribución:** concentración y posición acumulada.
### 5.4 Frame de ventana
```text
Fila actual = mes 4
ROWS BETWEEN 2 PRECEDING AND CURRENT ROW
[mes 2] [mes 3] [mes 4]
RANGE BETWEEN 1 PRECEDING AND CURRENT ROW
incluye filas cuyo valor de orden está entre (valor_actual - 1) y valor_actual
```
### 5.5 Errores comunes
| Error | Qué ocurre | Cómo evitarlo |
|---|---|---|
| Olvidar `ORDER BY` en acumulados | la suma no sigue secuencia temporal | ordenar por fecha o secuencia |
| Usar `LAST_VALUE()` sin frame completo | devuelve el valor de la fila actual y no el último total | usar `ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING` |
| Elegir mal `PARTITION BY` | comparas elementos de grupos incorrectos | revisar grano y pregunta del negocio |
| Filtrar con `WHERE` una función ventana | error semántico | usar subconsulta o `QUALIFY` |
> **📝 Nota:** La regla mental es: **ventana = quiénes participan (`PARTITION BY`), en qué orden (`ORDER BY`) y hasta dónde llega el cálculo (frame)**.

## 6. Explicación paso a paso
### Método recomendado
1. **Define el grano actual**: pedido, cliente-mes, proveedor-día, etc.
2. **Define la comparación**: contra sí mismo, contra su grupo o contra el histórico.
3. **Elige la partición**: ¿por segmento, región, cliente o proveedor?
4. **Elige el orden**: ¿fecha, monto, cantidad, prioridad?
5. **Define el frame**: ¿toda la historia, solo ventanas móviles o rango de valores?
6. **Valida el resultado** con pocas filas antes de escalar.
### Patrón mental
```text
Tabla base -> definir grano -> crear ventana -> calcular -> interpretar
```
### Cuándo usar cada familia
| Necesidad | Patrón recomendado |
|---|---|
| Ranking top N | `ROW_NUMBER`, `RANK`, `DENSE_RANK` + `QUALIFY` |
| Comparar con fila previa | `LAG` |
| Proyección siguiente | `LEAD` |
| Acumulado | `SUM() OVER (ORDER BY ... ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)` |
| Media móvil | `AVG() OVER (ORDER BY ... ROWS BETWEEN n PRECEDING AND CURRENT ROW)` |
| Participación porcentual | `valor / SUM(valor) OVER (...)` |
| Percentil relativo | `PERCENT_RANK`, `CUME_DIST`, `NTILE` |
> **📝 Nota:** Cuando una consulta con ventanas se vuelve compleja, usa una **CTE** para separar el cálculo base del cálculo analítico. Mejora legibilidad y reduce errores.
...
Ya viste seis ejemplos completos. Observa cómo cambió el comportamiento según la combinación de:
1. **Sin partición:** referencia global.
2. **Con partición:** referencia por grupo.
3. **Con orden:** referencia secuencial.
4. **Con frame:** referencia controlada en tamaño o rango.
> **📝 Nota:** En analítica real, la mayor parte de los errores con ventanas no están en la función (`SUM`, `AVG`, `LAG`), sino en una partición, orden o frame mal definidos.

## 8. Ejemplo guiado
En esta sección el objetivo es que leas la consulta como si estuvieras acompañando a un analista junior. Cada ejemplo está resuelto, pero debes intentar anticipar el resultado antes de ejecutarlo.
...
### Observaciones del ejemplo guiado
| Patrón | Valor para negocio |
|---|---|
| `% del total` | mide concentración |
| percentiles | mide posición relativa |
| `QUALIFY` | simplifica top N y filtrado post-ventana |
| `RANGE` vs `ROWS` | controla qué filas exactas participan |
| *feature engineering* | transforma historial en variables predictivas |
> **📝 Nota:** Si tu objetivo final es ML, las ventanas suelen ser el puente entre datos crudos y variables explicativas robustas.

## 9. Ejercicio guiado
A continuación verás ejercicios con progresión **Muy Fácil → Fácil → Intermedio → Intermedio Alto → Desafío**. Intenta resolverlos primero y luego compara con la solución de referencia incluida en la celda.
...
### Cierre del ejercicio guiado
Si resolviste los cinco ejercicios, ya dominas los patrones mínimos para:
- numerar,
- acumular,
- comparar contra el periodo previo,
- segmentar en grupos,
- y filtrar top N con `QUALIFY`.

## 10. Ejercicio individual
En esta sección debes intentar resolver primero por tu cuenta. La consulta mostrada es una **solución de referencia** para que valides tu razonamiento.
...
### Pista de auto-revisión
Antes de aceptar tu solución individual, pregúntate:
1. ¿La partición coincide con la pregunta?
2. ¿El orden temporal o numérico es el correcto?
3. ¿Necesito conservar el detalle o sí debía agrupar antes?
4. ¿Un `NULL` esperado en `LAG` o `LEAD` significa error o simplemente borde de la serie?

## 11. Desafío
Esta sección eleva la complejidad. Ahora combinaremos varias ventanas y varias capas de CTE para responder preguntas más cercanas a un entorno profesional.
...
## 12. Resumen
### Ideas clave
- `OVER()` convierte una función normal en una función de ventana.
- `PARTITION BY` define **quiénes compiten o se comparan**.
- `ORDER BY` define **la secuencia analítica**.
- `ROWS` usa posiciones físicas; `RANGE` usa distancia del valor de orden.
- `ROW_NUMBER`, `RANK`, `DENSE_RANK` y `NTILE` resuelven ranking y segmentación.
- `LAG`, `LEAD`, `FIRST_VALUE`, `LAST_VALUE` y `NTH_VALUE` conectan filas entre sí.
- `SUM`, `AVG`, `COUNT`, `MIN` y `MAX` sobre ventana crean acumulados, promedios y referencias de grupo.
- `QUALIFY` simplifica el filtrado posterior a funciones ventana en Databricks.
> **📝 Nota:** Si una pregunta necesita contexto relativo **sin perder la fila original**, probablemente necesites una window function.

## 13. Laboratorio
Responde las siguientes preguntas reales del CFO usando las consultas de referencia. El objetivo del laboratorio no es memorizar sintaxis, sino reconocer el patrón correcto.
### Preguntas del negocio
1. ¿Qué clientes muestran una tendencia descendente reciente en su gasto?
2. ¿Qué regiones concentran aproximadamente el 80% del ingreso?
3. ¿Qué proveedores son atípicos por saldo dentro de su región?
...
## 14. Autoevaluación
Responde mentalmente o por escrito antes de avanzar al siguiente notebook:
1. ¿Cuál es la diferencia central entre `GROUP BY` y una window function?
2. ¿Qué problema resuelve `PARTITION BY`?
3. ¿Por qué un acumulado necesita `ORDER BY` dentro de `OVER()`?
4. ¿Cuándo elegirías `ROWS` en lugar de `RANGE`?
5. ¿Qué diferencia práctica existe entre `ROW_NUMBER()`, `RANK()` y `DENSE_RANK()`?
6. ¿Qué devuelve `LAG()` cuando no existe una fila previa?
7. ¿Por qué `LAST_VALUE()` suele requerir un frame explícito?
8. ¿Cómo calcularías el porcentaje de una fila respecto al total sin perder la fila original?
9. ¿Qué ventaja ofrece `QUALIFY` en Databricks?
10. ¿Qué variables de comportamiento histórico podrías construir para un modelo de churn o propensión?
### Lista de verificación personal
| Pregunta | Sí | No |
|---|---|---|
| ¿Puedo explicar la anatomía de `OVER()`? | ☐ | ☐ |
| ¿Puedo construir un acumulado y una media móvil? | ☐ | ☐ |
| ¿Puedo interpretar percentiles y rankings? | ☐ | ☐ |
| ¿Puedo usar `QUALIFY` correctamente? | ☐ | ☐ |
| ¿Puedo diseñar features históricas con ventanas? | ☐ | ☐ |
> **📝 Nota:** Si aún dudas entre `GROUP BY` y ventanas, vuelve a los ejemplos 1 y 3: allí está la diferencia conceptual más importante del notebook.
