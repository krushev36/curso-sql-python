---
title: Notebook 06: Subconsultas y CTE
---

Fuente original: [06_subconsultas_cte.sql](https://github.com/krushev36/curso-sql-python/blob/main/sql/notebooks/06_subconsultas_cte.sql)

# Notebook 06: Subconsultas y CTE
## Fundamentos de Programación
### Maestría en Ciencia de Datos e Inteligencia de Negocios · Universidad de Antioquia
## 1. Bienvenida
Bienvenidas y bienvenidos al sexto notebook del curso **Fundamentos de Programación** de la **Maestría en Ciencia de Datos e Inteligencia de Negocios** de la **Universidad de Antioquia**.
En esta sesión asumirás el rol de **Data Analyst en DataCorp Analytics**. El equipo de analítica necesita responder preguntas de negocio que no se resuelven con una sola agregación: hace falta **descomponer el problema en pasos intermedios**, comparar contra promedios, validar existencia de registros relacionados y construir bloques reutilizables.
### Rol profesional del caso
| Elemento | Descripción |
|---|---|
| Empresa | DataCorp Analytics |
| Rol del estudiante | Data Analyst |
| Necesidad del negocio | Resolver preguntas analíticas multi-etapa |
| Herramientas centrales | Subconsultas, `EXISTS`, `IN`, `WITH` |
| Resultado esperado | Consultas legibles, correctas y reutilizables |
```text
Pregunta compleja -> Subpasos analíticos -> Validación lógica -> Resultado accionable
```
> **📝 Nota:** Una subconsulta no es “SQL avanzado por complejidad”, sino una forma ordenada de pensar: primero calculo algo intermedio, luego lo uso para decidir.

## 2. Objetivos de aprendizaje
Al finalizar este notebook podrás:
1. Explicar qué es una **subconsulta** y cuándo conviene usarla.
2. Aplicar subconsultas en `WHERE`, `FROM` y `SELECT`.
3. Distinguir correctamente entre subconsultas **correlacionadas** y **no correlacionadas**.
4. Resolver problemas de existencia con `EXISTS` y `NOT EXISTS`.
5. Comparar el uso de `IN` con subconsulta frente a `JOIN`.
6. Usar operadores `ANY` y `ALL` para comparaciones contra conjuntos.
7. Construir **CTE** simples, múltiples y encadenadas con `WITH`.
8. Reconocer cuándo una CTE mejora la **legibilidad**, la **modularidad** y la **mantenibilidad**.
9. Identificar escenarios donde conviene materializar resultados intermedios.
10. Interpretar casos de uso reales en negocio con subconsultas y CTE.
### Evidencias esperadas
| Habilidad | Evidencia |
|---|---|
| Descomponer problemas | consultas en varias capas |
| Elegir la técnica correcta | uso razonado de subconsulta o CTE |
| Evitar errores lógicos | control de duplicados, filtros y promedios |
| Comunicar resultados | aliases y comentarios claros |

## 3. Competencias
### Competencias técnicas
- Diseñar subconsultas escalares, de tabla y de existencia.
- Construir CTE nombradas para separar lógica analítica en módulos.
- Encadenar varias CTE sin perder el grano del análisis.
- Comparar conjuntos con `IN`.
### Competencias analíticas
- Traducir preguntas multi-etapa a una secuencia reproducible de pasos SQL.
- Justificar por qué una comparación debe hacerse contra un promedio global, local o por segmento.
- Detectar cuándo un `JOIN` puede duplicar filas y alterar la interpretación.
### Competencias profesionales
| Competencia | Aplicación en el trabajo |
|---|---|
| Claridad lógica | consultas auditables por otros analistas |
| Reusabilidad | bloques SQL fáciles de extender |
| Trazabilidad | resultados intermedios bien nombrados |
| Comunicación | explicación de supuestos y riesgos |
> **📝 Nota:** En equipos de datos maduros, escribir SQL legible es tan importante como escribir SQL correcto.

## 4. Contexto empresarial
El director de analítica de **DataCorp Analytics** ha pedido un notebook base para responder preguntas como:
1. ¿Qué clientes están **por encima del comportamiento promedio**?
2. ¿Qué proveedores participan en productos de **demanda superior a su categoría**?
3. ¿Qué regiones combinan **alto volumen**, **alto ticket** y **concentración de clientes estratégicos**?
4. ¿Cómo estructurar consultas largas sin convertirlas en un bloque difícil de mantener?
### Flujo de trabajo del analista
```text
Tablas base
    |
    v
Resultado intermedio 1 (promedio, lista, existencia)
    |
    v
Resultado intermedio 2 (filtrado o enriquecido)
    |
    v
Resultado final para decisión
```
### Tablas de trabajo de este notebook
| Dataset | Uso principal |
|---|---|
| `samples.tpch.customer` | clientes, segmento, saldo |
| `samples.tpch.orders` | pedidos, fechas, valor total |
| `samples.tpch.lineitem` | detalle de venta, cantidad, descuento |
| `samples.tpch.part` | producto, tipo, tamaño, precio |
| `samples.tpch.supplier` | proveedor |
| `samples.tpch.nation` | país |
| `samples.tpch.region` | región |
| `samples.nyctaxi.trips` | referencia opcional para extender ejercicios temporales |
> **📝 Nota:** Aunque el caso es ficticio, el patrón de trabajo es real: casi toda pregunta compleja en analítica requiere uno o varios pasos intermedios.

## 5. Conceptos
### 5.1 ¿Qué es una subconsulta?
Una **subconsulta** es una consulta dentro de otra consulta. Sirve para producir un valor, una lista o una tabla intermedia que luego usa la consulta principal.
### 5.2 Tipos principales
| Tipo | Devuelve | Lugar típico | Uso frecuente |
|---|---|---|---|
| Escalar | un solo valor | `SELECT`, `WHERE` | comparar contra promedio o total global |
| De lista | una columna con varios valores | `IN` | filtrar por pertenencia o comparación |
| De tabla | varias columnas y filas | `FROM` | crear una tabla derivada o vista inline |
| Correlacionada | depende de la fila externa | `WHERE`, `SELECT` | comparar cada fila con su propio grupo |
### 5.3 `EXISTS` y `NOT EXISTS`
- `EXISTS` devuelve verdadero si la subconsulta encuentra al menos una fila.
- `NOT EXISTS` devuelve verdadero si la subconsulta no encuentra filas.
- Suele ser ideal cuando la pregunta es: **“¿existe relación?”**
### 5.4 `IN` con subconsulta vs `JOIN`
| Técnica | Ventaja | Riesgo | Cuándo usar |
|---|---|---|---|
| `IN (subconsulta)` | semántica clara de pertenencia | puede requerir cuidado con `NULL` | cuando filtras por membresía |
| `JOIN` | permite traer columnas adicionales | puede duplicar filas | cuando además necesitas enriquecer el resultado |
### 5.5 Cuándo preferir CTE o subconsulta
| Si necesitas... | Conviene más |
|---|---|
| usar el resultado una sola vez y muy cerca del filtro | subconsulta |
| dividir la lógica en etapas legibles | CTE |
| reutilizar un bloque en varias partes de la misma consulta | CTE |
| expresar existencia puntual | `EXISTS` |
### 5.7 CTE recursivas y materialización
- **CTE recursiva:** se referencia a sí misma para recorrer jerarquías, secuencias o grafos simples.
- **Materializar una CTE:** cuando el resultado intermedio es costoso y se reutiliza muchas veces, puede convenir persistirlo como `TEMP VIEW` o tabla intermedia.
- En Databricks, una CTE suele ser una construcción lógica; el optimizador decide cómo ejecutarla.
> **📝 Nota:** La mejor pregunta no es “¿qué sintaxis me sé?”, sino “¿qué estructura hace más comprensible esta lógica dentro de mi equipo?”.

## 6. Explicación paso a paso
### Método recomendado para resolver preguntas multi-etapa
1. **Define la pregunta exacta.** ¿Buscas comparar, filtrar, detectar existencia o construir una tabla intermedia?
2. **Fija el grano.** ¿Una fila por cliente, pedido, región o producto?
3. **Decide el tipo de resultado intermedio.** Valor único, lista de llaves o tabla resumida.
4. **Elige la estructura.** Subconsulta escalar, correlacionada, tabla derivada o CTE.
5. **Valida duplicados.** Si un `JOIN` cambia el número de filas, quizás necesitabas `EXISTS`, `IN` o una agregación previa.
6. **Nombra bien los bloques.** Una CTE llamada `ingresos_cliente` comunica mejor que una subconsulta anónima larga.
7. **Comprueba el resultado esperado.** Antes de optimizar, verifica la lógica.
### Flujo lógico típico
```text
FROM/JOIN
   -> WHERE
   -> subconsulta o CTE intermedia
   -> SELECT final
   -> ORDER BY / LIMIT
```
### Errores comunes
...
|---|---|---|
| La subconsulta devuelve muchas filas cuando se esperaba una | se usó subconsulta escalar sin asegurar unicidad | agregar agregación o filtro |
| Un `JOIN` duplica clientes o pedidos | se unió contra detalle sin resumir antes | agregar `GROUP BY`, `DISTINCT` o usar `EXISTS` |
| La consulta es correcta pero ilegible | demasiada lógica anidada sin nombres | mover bloques a CTE |
| `NOT IN` falla con `NULL` | semántica de `NULL` en listas | preferir `NOT EXISTS` cuando hay duda |
> **📝 Nota:** En términos pedagógicos, una CTE es una forma de “poner nombre a un pensamiento intermedio”.

## 7. Ejemplo completamente explicado
En esta sección verás **5 ejemplos completos**. Cada consulta incluye el motivo de diseño, el papel de cada cláusula, el resultado esperado y errores comunes.
| Ejemplo | Dificultad | Idea central |
|---|---|---|
| 1 de 5 | Muy Fácil | subconsulta no correlacionada en `WHERE` |
| 2 de 5 | Fácil | subconsulta correlacionada en `WHERE` |
| 3 de 5 | Intermedio | subconsulta en `FROM` |
| 4 de 5 | Intermedio | subconsulta escalar en `SELECT` |
| 5 de 5 | Intermedio Alto | `EXISTS` y `NOT EXISTS` |
...
## 8. Ejemplo guiado
En esta sección ya no explicamos cada línea en un texto aparte, pero las consultas siguen comentadas y orientadas a la toma de decisiones.
| Ejemplo guiado | Enfoque | Nivel |
|---|---|---|
| 1 de 5 | `IN` con subconsulta | Muy Fácil |
| 2 de 5 | `ANY` | Fácil |
| 3 de 5 | `ALL` | Intermedio |
| 4 de 5 | CTE nombrada | Intermedio |
| 5 de 5 | múltiples CTE encadenadas | Intermedio Alto |
...
## 9. Ejercicio guiado
Ahora pasamos a ejercicios con solución de referencia. La progresión sigue la escala **Muy Fácil → Fácil → Intermedio → Intermedio Alto → Desafío**.
| Ejercicio guiado | Nivel | Habilidad principal |
|---|---|---|
| 1 de 5 | Muy Fácil | tabla derivada en `FROM` |
| 2 de 5 | Fácil | subconsulta escalar sobre CTE |
| 3 de 5 | Intermedio | subconsulta correlacionada |
| 4 de 5 | Intermedio Alto | `NOT EXISTS` |
| 5 de 5 | Desafío | múltiples CTE y porcentajes |
...
Recomendación didáctica: intenta resolver cada problema antes de ejecutar la solución de referencia. Aquí la solución está incluida para que puedas comparar tu razonamiento.
| Ejercicio individual | Nivel | Tema dominante |
|---|---|---|
| 1 de 5 | Muy Fácil | `IN` y pertenencia |
| 2 de 5 | Fácil | CTE temporal por mes |
| 3 de 5 | Intermedio | `EXISTS` vs duplicados |
| 4 de 5 | Desafío | top 3 países por ingreso y participación |
...
## 11. Desafío
Esta sección eleva el nivel de complejidad. Las soluciones siguen comentadas, pero el objetivo es que puedas leerlas como diseños analíticos completos.
| Desafío | Nivel | Tema dominante |
|---|---|---|
| 1 de 5 | Muy Fácil | clasificación con `EXISTS` |
| 2 de 5 | Fácil | CTE + promedio global |
| 3 de 5 | Intermedio | múltiples CTE con proveedores y regiones |
| 4 de 5 | Intermedio Alto | modularidad y reutilización de CTE |
| 5 de 5 | Desafío | introducción breve a CTE recursivas |
...
## 12. Resumen
### Ideas clave del notebook
- Una **subconsulta** sirve para producir un valor, una lista o una tabla intermedia.
- Las subconsultas en `WHERE` son ideales para comparar o filtrar.
- Las subconsultas en `FROM` son útiles cuando primero necesitas cambiar el grano del análisis.
- Las subconsultas escalares en `SELECT` agregan referencias globales o locales al resultado final.
- `EXISTS` y `NOT EXISTS` expresan mejor la lógica de existencia que muchos `JOIN`.
- `IN` es excelente para pertenencia; `JOIN` es mejor cuando además necesitas columnas adicionales.
- Las **CTE** mejoran legibilidad, modularidad y mantenibilidad.
- Si un bloque intermedio se reutiliza muchas veces o es costoso, evalúa **materializarlo**.
### Regla de decisión rápida
```text
¿Necesito un valor único?      -> subconsulta escalar
¿Necesito una lista?           -> IN
¿Necesito saber si existe?     -> EXISTS / NOT EXISTS
¿Necesito una tabla intermedia -> subconsulta en FROM o CTE
¿Necesito varias etapas claras -> CTE encadenadas
```
> **📝 Nota:** La técnica correcta es la que hace que el resultado sea correcto y el razonamiento sea visible para otra persona.

## 13. Laboratorio
A continuación desarrollarás consultas de estilo más profesional, pensadas como preguntas reales para el equipo de DataCorp Analytics.
| Caso | Pregunta de negocio |
|---|---|
| Laboratorio 1 | ¿Qué regiones concentran clientes de alto valor? |
| Laboratorio 2 | ¿Qué proveedores abastecen partes con demanda superior a su tipo? |
| Laboratorio 3 | ¿Qué segmentos combinan alto volumen y alto ticket? |
| Laboratorio 4 | ¿Qué países superan simultáneamente el promedio global y el regional? |
...
## 14. Autoevaluación
Usa esta sección para verificar si dominas la lógica del notebook.
### Preguntas de reflexión
1. ¿Cuándo una subconsulta debe ser correlacionada y cuándo no?
2. ¿Qué ventaja lógica ofrece `EXISTS` frente a un `JOIN` cuando solo te interesa saber si hay relación?
3. ¿Qué hace más legible a una CTE que a una subconsulta inline?
4. ¿Cuándo considerarías materializar una CTE en Databricks?
### Minicheck técnico
Ejecuta las siguientes consultas y explica con tus propias palabras por qué el resultado demuestra el concepto señalado.
