---
title: Notebook 04: Agregaciones
---

Fuente original: [05_agregaciones.sql](https://github.com/krushev36/curso-sql-python/blob/main/sql/notebooks/05_agregaciones.sql)

# Notebook 04: Agregaciones
## Fundamentos de Programación
### Maestría en Ciencia de Datos e Inteligencia de Negocios · Universidad de Antioquia
## 1. Bienvenida
Bienvenidos al cuarto notebook del curso **Fundamentos de Programación** de la **Maestría en Ciencia de Datos e Inteligencia de Negocios** de la **Universidad de Antioquia**.
En esta sesión aprenderás a **resumir datos** para responder preguntas de negocio reales: cuántos pedidos hubo, cuánto se vendió, qué regiones generan más ingresos y qué productos concentran el mayor volumen.
### Rol profesional del caso
| Elemento | Descripción |
|---|---|
| Empresa | DataCorp Analytics |
| Rol del estudiante | Data Analyst |
| Necesidad del negocio | Resúmenes ejecutivos y reportes para BI |
| Preguntas clave | Ingresos por región, ticket promedio por segmento, pedidos por mes, productos top |
> **📝 Nota:** En analítica, muchas decisiones no parten de filas individuales, sino de **agregaciones** que convierten millones de registros en indicadores comprensibles.

## 2. Objetivos de aprendizaje
Al finalizar este notebook podrás:
1. Aplicar funciones de agregación como `COUNT`, `SUM`, `AVG`, `MIN` y `MAX`.
2. Construir consultas con `GROUP BY` para resumir datos por una o varias dimensiones.
3. Filtrar resultados agregados con `HAVING` y diferenciarlo correctamente de `WHERE`.
4. Manejar casos con `NULL` y entender su impacto en los cálculos.
5. Generar subtotales y vistas multidimensionales con `ROLLUP`, `CUBE` y `GROUPING SETS`.
6. Calcular porcentajes y razones usando agregaciones y expresiones `CASE WHEN`.
### Resultado esperado
| Habilidad | Evidencia |
|---|---|
| Resumir datos | consultas con métricas claras |
| Analizar negocio | indicadores útiles para BI |
| Evitar errores | uso correcto de columnas agregadas y agrupadas |

## 3. Competencias
### Competencias técnicas
- Interpretar el **grano** de análisis antes de escribir una consulta.
- Seleccionar métricas adecuadas según la pregunta de negocio.
- Diseñar agregaciones robustas y legibles.
- Validar resultados agregados para evitar dobles conteos.
### Competencias analíticas
- Traducir preguntas ejecutivas a métricas SQL.
- Explicar resultados resumidos a audiencias no técnicas.
- Detectar patrones, concentraciones y anomalías.
### Competencias de buenas prácticas
| Buena práctica | Razón |
|---|---|
| Definir el nivel de agrupación antes de escribir | evita errores lógicos |
| Filtrar temprano con `WHERE` | mejora rendimiento y claridad |
| Filtrar grupos con `HAVING` | mantiene consistencia semántica |
| Etiquetar subtotales | hace más interpretable el resultado |

## 4. Contexto empresarial
El equipo de Business Intelligence de **DataCorp Analytics** necesita un notebook reutilizable para responder cuatro solicitudes frecuentes:
1. **Ingresos totales por región** para decidir dónde enfocar campañas.
2. **Valor promedio de pedido por segmento de cliente** para priorizar cuentas.
3. **Número de pedidos por mes** para analizar estacionalidad.
4. **Productos líderes por volumen de venta** para optimizar inventario.
### Flujo de decisión
```text
Filas transaccionales -> Agrupar -> Calcular métricas -> Comparar -> Decidir
```
### Tablas de trabajo en este notebook
| Dataset | Uso principal |
|---|---|
| `samples.tpch.orders` | pedidos y valor total |
| `samples.tpch.customer` | segmento y cliente |
| `samples.tpch.lineitem` | detalle de venta y cantidades |
| `samples.tpch.part` | información de producto |
| `samples.tpch.supplier` | proveedores |
| `samples.tpch.nation` | país |
| `samples.tpch.region` | región |
| `samples.nyctaxi.trips` | ejemplo adicional de agregación temporal si se desea extender |
> **📝 Nota:** Aunque el negocio es ficticio, las preguntas son idénticas a las que un analista resuelve en producción.

## 5. Conceptos
### 5.1 Funciones de agregación
| Función | Qué hace | Consideración |
|---|---|---|
| `COUNT(*)` | cuenta filas | incluye filas con `NULL` |
| `COUNT(columna)` | cuenta valores no nulos | ignora `NULL` |
| `COUNT(DISTINCT columna)` | cuenta valores únicos no nulos | puede ser más costoso |
| `SUM(columna)` | suma valores | ignora `NULL` |
| `AVG(columna)` | promedio | ignora `NULL` |
| `MIN(columna)` | mínimo | ignora `NULL` |
| `MAX(columna)` | máximo | ignora `NULL` |
### 5.2 ¿Qué hace `GROUP BY`?
```text
Antes del GROUP BY
pedido  cliente  región   valor
1       A        AMERICA  100
2       B        AMERICA  300
3       C        EUROPE   200
Después del GROUP BY región
región   suma_valor
AMERICA  400
EUROPE   200
```
...
|---|---|---|
| Columna en `SELECT` no agregada | SQL no sabe qué valor mostrar por grupo | agregarla a `GROUP BY` o resumirla |
| Usar `HAVING` para filtros de detalle | funciona, pero no es la intención correcta | mover el filtro a `WHERE` |
| Dobles conteos por joins | una fila se replica al unir tablas detalle | revisar el grano antes de agregar |
> **📝 Nota:** La regla mental es: **toda columna en `SELECT` debe estar agregada o incluida en `GROUP BY`**.

## 6. Explicación paso a paso
### Método recomendado para construir una agregación
1. **Define la pregunta**: ¿quieres contar, sumar, promediar o comparar?
2. **Define el grano**: por región, por mes, por producto, por segmento.
3. **Identifica tablas y joins**: evita mezclar niveles de detalle sin control.
4. **Aplica `WHERE`** para filtrar filas antes del resumen.
5. **Aplica `GROUP BY`** para establecer una fila por categoría.
6. **Aplica `HAVING`** si el filtro depende del resultado agregado.
7. **Ordena** con `ORDER BY` para priorizar el análisis.
### Distinguir `WHERE` vs `HAVING`
| Cláusula | Momento lógico | Uso correcto |
|---|---|---|
| `WHERE` | antes de agrupar | filtrar filas base |
| `HAVING` | después de agrupar | filtrar grupos ya resumidos |
### Regla práctica
```text
FROM/JOIN -> WHERE -> GROUP BY -> HAVING -> SELECT -> ORDER BY
```
> **📝 Nota:** Si tu condición usa `SUM`, `COUNT` o `AVG`, probablemente pertenece a `HAVING`.
...
En esta sección resolvemos cinco consultas modelo, cada una alineada con una necesidad frecuente del negocio.
| Ejemplo | Pregunta | Técnica principal |
|---|---|---|
| 1 | ¿Cuánto ingresa cada región? | `SUM` + `GROUP BY` |
| 2 | ¿Cuál es el ticket promedio por segmento? | `AVG` + joins |
| 3 | ¿Cuántos pedidos ocurren por mes? | agrupación temporal |
| 4 | ¿Qué productos lideran por volumen? | `SUM` + ordenamiento |
| 5 | ¿Qué países tienen suficientes proveedores para analizar? | `HAVING` |
> **📝 Nota:** Lee la explicación antes de ejecutar cada consulta y compara el resultado con la expectativa indicada.
...
## 8. Ejemplo guiado
A continuación verás cinco consultas con una lógica muy parecida a la que usarías en un taller acompañado por el docente.
### Estrategia de trabajo
1. Lee la pregunta.
2. Identifica la dimensión de agrupación.
3. Elige la métrica.
4. Verifica si el filtro pertenece a `WHERE` o a `HAVING`.
5. Revisa si existe riesgo de doble conteo por joins.
| Ejercicio guiado | Dificultad | Enfoque |
|---|---|---|
| 1 | Muy fácil | `COUNT` por región |
| 2 | Fácil | `SUM` por año y segmento |
| 3 | Fácil | `HAVING` por producto |
| 4 | Intermedio | `CASE WHEN` + porcentajes |
| 5 | Intermedio | `ROLLUP` temporal |
...
## 9. Ejercicio guiado
En esta sección debes intentar resolver primero por tu cuenta y luego comparar con la solución propuesta.
### Progresión de dificultad
| Nivel | Objetivo |
|---|---|
| Muy fácil | practicar conteos básicos |
| Fácil | combinar `SUM` y `AVG` con una dimensión |
| Intermedio | usar `HAVING` con umbrales |
| Intermedio alto | resumir detalle de ventas por producto |
| Desafío | calcular participaciones porcentuales mensuales |
> **📝 Nota:** Si recibes el error *"expression is neither present in the group by, nor is it an aggregate function"*, revisa inmediatamente las columnas del `SELECT`.
...
Ahora trabaja de manera autónoma. La idea es que construyas las consultas antes de mirar la solución de referencia de la celda siguiente.
### Recomendaciones
- Escribe primero el `SELECT` con la métrica.
- Luego decide si la dimensión pertenece al `GROUP BY`.
- Si usas joins, pregúntate: **¿cambia el número de filas?**
- Comprueba si tus porcentajes suman aproximadamente 100.
| Ejercicio | Nivel | Resultado esperado |
|---|---|---|
| 1 | Muy fácil | ingreso por región y año |
| 2 | Fácil | descuento promedio por región |
| 3 | Intermedio | productos distintos por país proveedor |
| 4 | Intermedio alto | años con ticket promedio alto |
| 5 | Desafío | resumen con `GROUPING SETS` |
...
## 11. Desafío
Estas consultas elevan el nivel analítico. Intenta entender la lógica de negocio antes de ejecutarlas.
### Enfoques avanzados de esta sección
| Desafío | Concepto dominante |
|---|---|
| 1 | porcentaje de participación por región y segmento |
| 2 | `ROLLUP` temporal con ventas |
| 3 | `CUBE` de región y prioridad |
| 4 | `GROUPING SETS` con marca de producto |
| 5 | comparación contra promedio global |
> **📝 Nota:** Un buen desafío no es solo escribir SQL que corra; es escribir SQL que entregue una historia clara para la toma de decisiones.
...
## 12. Resumen
### Ideas clave de este notebook
- `COUNT(*)` cuenta filas; `COUNT(columna)` ignora `NULL`.
- `COUNT(DISTINCT)` sirve para cardinalidad única, no para volumen bruto.
- `GROUP BY` define el **nivel de detalle** del resultado.
- `HAVING` filtra grupos; `WHERE` filtra filas antes de agrupar.
- `ROLLUP` agrega subtotales jerárquicos.
- `CUBE` crea todas las combinaciones de subtotales entre dimensiones.
- `GROUPING SETS` ofrece control explícito de qué resúmenes generar.
- `CASE WHEN` es ideal para construir categorías analíticas antes de agregar.
- Los porcentajes requieren una base claramente definida: por grupo, por subtotal o por total general.
### Lista de chequeo mental
| Pregunta | Sí/No |
|---|---|
| ¿Definí el grano del análisis? |  |
| ¿Toda columna del `SELECT` está agregada o en `GROUP BY`? |  |
| ¿Los filtros de detalle van en `WHERE`? |  |
| ¿Los filtros de grupo van en `HAVING`? |  |
| ¿Validé el impacto de `NULL` y de los joins? |  |

## 13. Laboratorio
### Preguntas reales de negocio para DataCorp Analytics
1. ¿Qué región concentra la mayor parte del ingreso neto y qué porcentaje representa sobre el total?
2. ¿Qué segmento de cliente tiene el ticket promedio más alto en cada región?
3. ¿En qué meses cae el volumen de pedidos y cómo cambia el ingreso mensual?
4. ¿Qué marcas o productos combinan alto volumen con alto ingreso?
5. ¿Qué prioridades de pedido dominan en cada región?
6. ¿Qué países proveedores tienen suficiente escala y diversidad de productos?
7. ¿Qué subtotales conviene mostrar en el dashboard ejecutivo: por región, por segmento o ambos?
### Sugerencia de dinámica de laboratorio
| Paso | Acción |
|---|---|
| 1 | Formular la pregunta en lenguaje de negocio |
| 2 | Definir la métrica y el grano |
| 3 | Seleccionar tablas y joins |
| 4 | Escribir la agregación |
| 5 | Validar resultados y explicar hallazgos |
## 14. Autoevaluación
Marca si puedes resolver cada punto sin ayuda:
- [ ] Explico la diferencia entre `COUNT(*)` y `COUNT(columna)`.
- [ ] Sé cuándo usar `WHERE` y cuándo usar `HAVING`.
- [ ] Puedo corregir un error por columna no agregada.
- [ ] Puedo crear resúmenes por dos o más dimensiones.
- [ ] Puedo calcular un porcentaje sobre el total con agregaciones.
- [ ] Puedo usar `ROLLUP`, `CUBE` y `GROUPING SETS` según la necesidad.
- [ ] Puedo justificar por qué una consulta responde una pregunta real de BI.
> **📝 Nota:** Si todavía dudas en alguno de los puntos, vuelve a las secciones 6 a 11 y repite las consultas cambiando la dimensión de análisis.
